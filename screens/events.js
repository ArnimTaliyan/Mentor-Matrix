...
export const generateWeeklyEvents = (startDate, numberOfWeeks) => {
  ...
  for (let i = 1; i <= numberOfWeeks; i++) { // Start from 1 instead of 0
    ...
    events.push(
      ...
      {
        title: "CCVT",
        start: new Date(startOfWeek.getFullYear(), startOfWeek.getMonth(), startOfWeek.getDate() + 4, 1, 0), // Tuesday, 1:00 PM
        end: new Date(startOfWeek.getFullYear(), startOfWeek.getMonth(), startOfWeek.getDate() + 4, 2, 50),  // Tuesday, 2:50 PM
        location: "Classroom 11112",
        Teacher: "Ambika Aggarwal"
      },
...
let eventsData = generateWeeklyEvents(new Date(2024, 4, 13), 4);
